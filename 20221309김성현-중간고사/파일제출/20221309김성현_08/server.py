...
s.sendto(b'Hello',addr)  # 'Hello' 메시지 전송

msg = s.recvfrom(BUF_SIZE)  # 'Filename' 메시지 수신
...
if not msg:
    ...
    s.sendto(filename.encode())  # 파일 이름 전송

    msg = s.recvfrom(BUF_SIZE)  # 파일 크기 수신
    ...
    if not msg:
        ...
        print('server:', filesize)  # 4바이트로 받은 파일 크기
